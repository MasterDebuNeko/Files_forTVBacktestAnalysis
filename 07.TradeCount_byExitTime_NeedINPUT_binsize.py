...
# Ensure required columns exist (now checking for 'Exit Time' too)
required_cols = ['Exit Time', 'Profit(R)']
if not all(col in df.columns for col in required_cols):
    missing = [col for col in required_cols if col not in df.columns]
    # Adjusted error message to reflect checking for Exit Time
    raise KeyError(f"คอลัมน์ที่จำเป็นไม่ครบถ้วน: {missing}. ตรวจสอบ calc_r_multiple_and_risk และว่ามีคอลัมน์ Exit Time.")


# Prepare data for plotting
# Filter out rows with missing Exit Time (previously was Entry Time) and create a copy
df = df[df['Exit Time'].notnull()].copy()

# Ensure Exit Time and Profit(R) are correct types
df['Exit Time'] = pd.to_datetime(df['Exit Time']) # Use Exit Time
df['Profit(R)'] = df['Profit(R)'].astype(float)

# --- Create Exit Time of Day (Time within the day) ---
# Extract just the time part and represent it as seconds from midnight for easier binning
# Use Exit Time for calculation
df['Exit Time of Day'] = (df['Exit Time'].dt.hour * 3600 +
                          df['Exit Time'].dt.minute * 60 +
                          df['Exit Time'].dt.second) # Time in seconds from midnight
...
bin_size_minutes = 60 # Keeping 60 minutes as the bin size for this example
...
# Create labels for the bins (e.g., "00:00-01:00", "01:00-02:00", ...)
bin_labels = []
...
# Assign each trade to a time bin using Exit Time of Day
df['Time Bin'] = pd.cut(df['Exit Time of Day'], bins=bins, labels=bin_labels, right=False, include_lowest=True)


# --- Count trades by Time Bin and Result Type (for all bins initially) ---
# Group by the new 'Time Bin' based on Exit Time
...
# --- Filter Data for the Desired Time Range (Applying to Exit Time) ---
...
# --- Plotting (Exit Time of Day - Skipped Range) ---
# Updated title to reflect Exit Time
print("Trade Counts by Exit Time of Day (60-min bins, 12:00-19:30 skipped) and Result Type - Chart")
# Adjusted figure size - Slightly wider
...
ax.set_xlabel('Exit Time of Day (60-min bins)') # Updated x-axis label
ax.set_ylabel('Number of Trades')
# Updated title
ax.set_title('Trade Counts by Exit Time of Day (60-min bins, 12:00-19:30 Skipped) and Result Type - Chart')
ax.set_xticks(x_filtered) # Use the new x-positions for ticks
# Rotate labels - Back to 45 degrees and right alignment
ax.set_xticklabels(bin_labels_filtered, rotation=45, ha='right') # Using 45deg rotation and right alignment
...
if start_skip_index > 0 and start_display_index < len(bin_labels) and start_skip_index <= start_display_index:
    ...
    # Updated label for the line
    ax.axvline(line_x_position, color='gray', linestyle='--', linewidth=1.5, label='Skipped Exit Time (12:00 - 19:30)')
    ...
    # Updated text annotation
    ax.text(annotation_x, annotation_y, '  Skipped Exit\n12:00 - 19:30',
    ...
    try:
        # Updated label to find in legend
        skipped_label_index = labels.index('Skipped Exit Time (12:00 - 19:30)')
     ...
     print(f"Info: Skipping Exit Time from the beginning of the day until {bin_labels[start_display_index].split('-')[0]}. No vertical line needed before the displayed data.")
elif start_skip_index > 0 and start_display_index >= len(bin_labels):
     print(f"Info: Skipping Exit Time from {bin_labels[start_skip_index].split('-')[0]} until the end of the day. No vertical line needed after the displayed data.")
elif start_skip_index == 0 and start_display_index >= len(bin_labels):
     print("Info: Skipped range covers all Exit Times. No data to plot.")
else:
     print("Info: No skipped range identified based on Exit Times 12:00 and 19:30. All bins displayed if data exists.")
...
# Updated title to reflect Exit Time
print("\nSummary Table: Trade Counts and Percentage by Exit Time of Day (60-min bins, 12:00-19:30 skipped)")
