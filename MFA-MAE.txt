...
required_cols = ['MFE(R)', 'MAE(R)', 'Profit(R)']
...
# Filter out rows where MFE(R) or MAE(R) are NaN, as they can't be plotted
df_plot = df.dropna(subset=['MFE(R)', 'MAE(R)']).copy()

# Handle empty dataframe case after dropping NaNs
if df_plot.empty:
    print("ℹ️ ไม่มีข้อมูลที่มีค่า MFE(R) และ MAE(R) ที่ถูกต้อง ไม่สามารถสร้างกราฟ scatter ได้")
else:
    ...
    scatter = plt.scatter(df_plot['MFE(R)'], df_plot['MAE(R)'], c=colors, alpha=0.6, s=20) # s is marker size

    # Add labels and title
    plt.xlabel('MFE (R)')
    plt.ylabel('MAE (R)')
    plt.title('MFE vs MAE by Trade Outcome')
